...
export const Mockup = observer(
  ({ inputsWithId, parentId, store }: MockupProps) => {
    ...
    const [modalDataElementType, setModalDataElementType] = useState<string>();
    ...
    const filterPendingFieldsOnUsage = (
      ...
      const filteredPendingFields = pendinNewFields.filter(({ keyname }) => {
        const grabbedFieldValues = Object.entries(store.grabbedInput?.data)
    ...
    return (
      <>
        ...
        <Modal
          ...
          afterOpenChange={() => {
            setModalDataElementType(store?.grabbedInput?.value.type);
            store.setDragging(false);
          }}
          ...
          onOk={() => {
            ...
            store.setFields([...store.fields, ...filteredPendingFields]);
