...
export const PropertiesPanel = observer(
  ({ store }: { store: FormbuilderEditorStore }) => {
    ...
    useEffect(() => {
      ...
      const fieldsPartData = Object.fromEntries(fieldPropsPartDataEntries);
    ...
    const currentInputSchema =
      elementsData.find((el) => el.elementId === currentInputType)?.schema ||
      {};
    ...
    const updateFieldInSelectedInput = (
      ...
      if (currentSelectedInput && currentSelectedInput.id) {
        ...
        form.validateFields().then((result) => {
          store.setNewElementData(currentSelectedInput.id as number, result);
        });
      }
    };
        ...
        case "field":
          return (
            <FieldSelectInput
              ...
              updateFieldInSelectedInput={updateFieldInSelectedInput}
            />
          );
        case "boolean":
          return <Checkbox />;
            ...
            <>
              ...
              >
                {fieldsFromSchema.map((field, i) => (
                  ...
                  >
                    {store.selectedInput &&
                      getPropertiesFormInput(field, store.selectedInput)}
                  </Form.Item>
                ))}
              </Form>
              <div>
