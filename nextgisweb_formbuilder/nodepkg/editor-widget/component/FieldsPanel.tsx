    ...
    const deleteField = (field: FormbuilderEditorField) => {
      ...
      if (isFieldOccupied(field.keyname, store.inputsTree)) {
        ...
        if (elIdtoCleanField >= 0) {
          const elementToClean = store.getElementById(elIdtoCleanField);

          const dataEntries = Object.entries(elementToClean?.data);

          const updatedDataEntries = dataEntries.map(([key, value]) => {
    ...
    const getStatusIcon = (field: FormbuilderEditorField) => {
      ...
      if (field.existing) {
        ...
        return store.updateFeatureLayerFields ? (
          <NewUsedFieldIcon style={{ color: "var(--theme-color-success)" }} />
        ) : (
          <ErrorIcon style={{ color: "var(--theme-color-warning)" }} />
        );
      } else if (isUsed && !store.canUpdateFields) {
        return <ErrorIcon style={{ color: "var(--theme-color-warning)" }} />;
      } else if (!isUsed) {
        return <ErrorIcon style={{ color: "var(--theme-color-warning)" }} />;
      } else {
        return <></>;
      ...
      >
        ...
        <div className="panel-body">
          {store.fields.map((field, i) => {
            const datatypeSelectOptions = getFilteredFieldDataTypeOptions(
              field.keyname
            );

            const defaultValue =
              datatypeSelectOptions.find((opt) => !opt.disabled)?.value ||
              datatypeSelectOptions[0].value;
