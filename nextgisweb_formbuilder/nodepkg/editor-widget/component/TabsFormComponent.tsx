...
export const TabsFormComponent = observer(
  ({
    ...
    const addTab = () => {
      ...
      store.setNewElementValue(value.id as number, updatedTabsInput.value);
    ...
    const setActive = (currentTabIndex: number) => {
      ...
      const updatedTabs = deepClonedTabs.map((tab: UITab, index: number) => {
            ...
            <div
              ...
              onMouseUp={() => {
                ...
                if (
                  ...
                  value.id &&
                  store.isNestedWithin(store.grabbedInput.id, value.id)
                ) {
                  return;
                }
                ...
                const getDroppingFieldValue = () => {
                  ...
                  if (isNonFieldElement(store?.grabbedInput)) {
                    return undefined;
                  }
                ...
                if (!store.isMoving && !isNonFieldElement(droppedInput)) {
                ...
                const freshTabs =
                  store.getElementById(tabsInputFromStore?.id as number)?.value
                    .tabs || [];

                const updatedTabs = freshTabs.map((tab, index) => {
                  if (index === i) {
                    return {
                      ...tab,
                      items: {
